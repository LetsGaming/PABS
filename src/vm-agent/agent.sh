...
do_install() {
    # ERR trap fires on any non-zero subcommand exit under set -E. During
    # install, several benign commands (e.g. grep, test) may return non-zero.
    # Clear the trap for the duration of this function to avoid false fatals.
    trap - ERR

    local detected
    detected=$(detect_type)

    # Always write the config next to the agent script itself.
    # AGENT_CONFIG was resolved at startup by checking if the file existed —
    # but at install time it doesn't exist yet, so that check fell back to
    # /etc/pabs-agent/config. We bypass that here and derive the path directly
    # from the script's own location, which is the correct persistent path on
    # all platforms (e.g. /config/.pabs-agent/config on HAOS).
    local install_config
    install_config="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/config"
...
EOF
    ...
    log "Config written to $install_config (mode 600)"
    log ""
